    ...
    public async _request<T>(path: string, method: string = 'GET', body?: any): Promise<T> {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                // [SECURITY] Don't leak stack traces in API errors
            ...
            if (error.name === 'AbortError') {
                throw new Error(`API Error: Request timeout after 30 seconds`);
            }
            throw error;
        } finally {
    ...
    public async downloadResults(options: { 
        ...
        const download = async (url: string, name: string) => {
            ...
            try {
                ...
                if (error.name === 'AbortError') {
                    throw new Error(`Download Error: Request timeout after 30 seconds for ${url}`);
                }
                throw error;
            } finally {
                clearTimeout(timeout);
