...
export class Visual {
    ...
    public async _request<T>(path: string, method: string = 'GET', body?: any): Promise<T> {
        ...
        // [SECURITY] Enforce a 30-second timeout on external API calls to prevent hanging/DoS
...
export class JobHandle {
    ...
    public async downloadResults(options: { 
        ...
        const download = async (url: string, name: string) => {
            // [SECURITY] Enforce a 30-second timeout on downloading images to prevent hanging/DoS
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), 30000);
