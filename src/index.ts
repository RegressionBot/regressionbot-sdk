...
export class Visual {
    ...
    public async _request<T>(path: string, method: string = 'GET', body?: any): Promise<T> {
        ...
        // [SECURITY] Prevent hanging/DoS by enforcing a timeout on external API calls
...
export class JobHandle {
    ...
    public async downloadResults(options: { 
        ...
        const download = async (url: string, name: string) => {
            // [SECURITY] Prevent hanging/DoS by enforcing a timeout on external file downloads
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 10000);
