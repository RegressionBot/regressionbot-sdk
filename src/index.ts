    ...
    public async downloadResults(options: { 
        ...
        const download = async (url: string, name: string) => {
            try {
                ...
                if (!res.body) {
                    console.warn(`Warning: Empty response body for ${name}`);
                    return;
                }
                ...
                // 🛡️ SECURITY: Stream response to disk to prevent Memory Exhaustion (DoS)
                await pipeline(Readable.fromWeb(res.body as ReadableStream), dest);
            } catch (err: any) {
