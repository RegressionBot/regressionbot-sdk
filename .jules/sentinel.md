...
## 2025-04-07 - [Hanging/DoS vulnerability in fetch requests]
**Vulnerability:** The `fetch` calls in `src/index.ts` within `_request` and `downloadResults` lacked timeouts. This could cause the application to hang indefinitely if the server responds slowly or not at all, leading to potential Denial of Service (DoS). Additionally, returning `response.json()` without `await` inside a `try/catch` block meant that errors during the parsing phase would not be caught by the block.
**Learning:** External API calls must always have timeouts to prevent hanging and resource exhaustion. When returning a promise from inside a `try/catch` block, it must be `await`ed to ensure errors are properly caught and handled within that scope.
**Prevention:** Always implement an `AbortController` with a `setTimeout` for external `fetch` calls, ensuring the timeout is cleared in a `finally` block. Use `return await` when returning promises from `try/catch` blocks.
