...
## 2025-03-01 - [Missing Timeout in External Fetch Calls]
**Vulnerability:** The SDK used the global `fetch` API for making internal API requests and downloading images without explicitly setting a timeout. Node.js `fetch` has no default timeout, which could cause requests to hang indefinitely.
**Learning:** Using `fetch` without an `AbortController` and timeout can lead to Denial of Service (DoS) risks or application hangs when communicating with slow or unresponsive endpoints.
**Prevention:** Always wrap `fetch` calls with an `AbortController`, implement a reasonable timeout (e.g., 30 seconds), and ensure the timeout is cleared in a `finally` block to prevent memory leaks or delayed promise rejections.
